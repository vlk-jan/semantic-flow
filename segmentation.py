...
class Segmentation:
    ...
    def segment_scene(self, scene_dir: Union[str, Path], sensors: List[str]) -> None:
        ...
        for sensor in tqdm.tqdm(sensors):
            ...
            points = np.array(list([h, w] for h in rand_h for w in rand_w), dtype=np.float32)
            ...
            for out_frame_idx in range(0, len(frame_names), vis_frame_stride):
                ...
                Path(f"/mnt/personal/vlkjan6/renders/{scene_dir.stem}/{sensor}").mkdir(parents=True, exist_ok=True)
                plt.savefig(f"/mnt/personal/vlkjan6/renders/{scene_dir.stem}/{sensor}/{out_frame_idx}")
