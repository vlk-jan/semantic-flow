...
class Segmentation:
    ...
    def plot_image(
        ...
        masks: Union[List[Dict[str, any]], None] = None,
    ) -> None:
        """
    ...
    def create_segmented_image(
        self, masks: Optional[List[Dict[str, any]]] = None
    ) -> np.ndarray:
        """
    ...
    def segment_image(
        self, image: Optional[Union[np.ndarray, str]] = None
    ) -> List[Dict[str, any]]:
        """
        Segments image.
    ...
    def segment_scene(self, scene_dir: Union[str, Path], sensors: List[str]) -> None:
        ...
        for sensor in tqdm.tqdm(sensors):
            h_p, w_p = (6, 8) if sensor != "ring_front_center" else (8, 6)
            height, width = (
                (1550, 2048) if sensor != "ring_front_center" else (2048, 1550)
            )
            rand_h = np.random.randint(0, height, h_p)
            rand_w = np.random.randint(0, width, w_p)
            points = np.array(
                list([h, w] for h in rand_h for w in rand_w), dtype=np.float32
            )
            ...
            for out_frame_idx in range(0, len(frame_names), vis_frame_stride):
                ...
                Path(f"/mnt/personal/vlkjan6/renders/{scene_dir.stem}/{sensor}").mkdir(
                    parents=True, exist_ok=True
                )
                plt.savefig(
                    f"/mnt/personal/vlkjan6/renders/{scene_dir.stem}/{sensor}/{out_frame_idx}"
                )
