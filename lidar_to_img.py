...
class LidarToImg:
    def __init__(
        self, scene: Union[AVScene, Path, str], sensors: Optional[List[str]] = None
    ):
        ...
        self.sensors = (
            ...
            if sensors is not None
                ...
                sensor
                for sensor in self.scene.camera_intrinsics.keys()
                if "stereo" not in sensor
            ]
        )

    def run(
        self,
        save_dir: Union[Path, dir],
        seg_img_path: Union[Path, str],
        ...
        for timestamp in tqdm.tqdm(self.scene.pcd_timestamps):
            ...
            feather.write_feather(
                pcd, save_dir / self.scene.root_dir.stem / f"{timestamp}.feather"
            )
    ...
    def color_pcd(self, timestamp: int, seg_img_path: Union[Path, str]) -> pd.DataFrame:
        ...
        pcd = feather.read_feather(
            self.scene.pcd[self.scene.pcd_timestamps.index(timestamp)]
        )
        full_pcd = np.c_[
            pcd["x"],
            pcd["y"],
            pcd["z"],
            np.zeros(pcd.shape[0]),
            np.zeros(pcd.shape[0]),
            np.zeros(pcd.shape[0]),
            np.zeros(pcd.shape[0]),
        ]
    ...
    def _project_lidar_to_img(self, timestamp: int) -> Dict[str, np.ndarray]:
        ...
        for sensor in self.sensors:
            pc_into_img = (
                np.linalg.pinv(self.scene.sensor_extrinsics[sensor]) @ full_pcd.T
            )
            ...
            pixel_points_3d = (
                pc_into_img
                @ self.scene.camera_T_view
                @ self.scene.camera_intrinsics[sensor].T
            )
    ...
    def visualize(
        ...
        for sensor in self.sensors:
            ...
            pixel_points_2d = np.round(
                pixel_points_3d[:, :2] / pixel_points_3d[:, 2:]
            ).astype(int)
            ...
            projected_rgb_image = self._rescale_img(
                imgs[sensor].copy(), reduction_scale
            )
            projected_rgb_image[pixel_points_2d[:, 1], pixel_points_2d[:, 0], :] = (
                colors * 255
            )
    ...
    def _color_pcd_distance(
        self, x: np.ndarray, max_distance: float = 10, cmap: str = "viridis"
    ) -> np.ndarray:
        """
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--scene_dir",
        type=str,
        default="./datasets/ff8e7fdb-1073-3592-ba5e-8111bc3ce48b.pkl",
    ...
    parser.add_argument(
        "--seg_dir", type=str, default="./seg", help="Path to the segmented images"
    )
    parser.add_argument(
        "--vis_3d",
        action="store_true",
        help="Flag to visualize the colored point cloud",
    )
    parser.add_argument(
        "--vis_img",
        action="store_true",
        help="Flag to visualize the projected point cloud",
    )
